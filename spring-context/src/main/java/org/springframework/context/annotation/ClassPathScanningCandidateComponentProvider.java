...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
    ...
    private boolean indexSupportsIncludeFilter(TypeFilter filter) {
        if (filter instanceof AnnotationTypeFilter) {
            ...
            return (AnnotationUtils.isAnnotationDeclaredLocally(Indexed.class, annotation) || annotation.getName()
                    .startsWith("javax."));
        }
    ...
    private String extractStereotype(TypeFilter filter) {
        if (filter instanceof AnnotationTypeFilter) {
            return ((AnnotationTypeFilter) filter).getAnnotationType()
                    .getName();
        }
        if (filter instanceof AssignableTypeFilter) {
            return ((AssignableTypeFilter) filter).getTargetType()
                    .getName();
        }
        return null;
    }
            ...
            for (Resource resource : resources) {
                ...
                if (resource.isReadable()) {
                    try {
                        ...
                        // excludeFilters + includeFilters + @Conditional 判断
                        if (isCandidateComponent(metadataReader)) { // @Component --> includeFilters 判断
                            // 包装成为一个 ScannedGenericBeanDefinition
                            ScannedGenericBeanDefinition sbd = new ScannedGenericBeanDefinition(metadataReader);
                            // 并且设置 class资源
                            sbd.setSource(resource);
                            // 是不是候选的组件（是独立类 && ( (不是接口 && 不是抽象类 ) || 是抽象类有@Lookup注解)）
                            if (isCandidateComponent(sbd)) {
                                if (debugEnabled) {

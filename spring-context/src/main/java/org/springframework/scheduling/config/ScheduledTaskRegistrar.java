...
public class ScheduledTaskRegistrar implements ScheduledTaskHolder, InitializingBean, DisposableBean {
    ...
    /**
     * 触发器任务。最终是使用JDK的延时执行一次来实现的，而执行的run方法会执行 schedule 方法，
     * schedule 的实现就是,拿到 {@link Trigger#nextExecutionTime(TriggerContext)} 与当前时间相减得出延时时间。
     *  {@link org.springframework.scheduling.concurrent.ReschedulingRunnable#run()}
     *
    ...
    /**
     * 定时器任务。与 TriggerTask 是一样的逻辑
     */
    @Nullable
    private List<CronTask> cronTasks;

    /**
     * 周期任务。最终是使用JDK的周期任务实现的。
    ...
    /**
     * 未解析的任务，就是还没有开始调度的任务，调度之后就会删掉了
     */
    private final Map<Task, ScheduledTask> unresolvedTasks = new HashMap<>(16);

    /**
     * 已调度的任务集合。作用就是用来销毁定时任务
     *  ScheduledTask 其实就是 TriggerTask、CronTask、IntervalTask 的装饰对象，这几个Task就是要运行的任务,还包含了 ScheduledFuture 用来中断定时任务
     */
    ...
    protected void scheduleTasks() {
        ...
        /**
         * 使用 taskScheduler 调度任务，然后将调度的任务信息记录在 ScheduledTask 属性中
         * */
        if (this.triggerTasks != null) {
    ...
    public ScheduledTask scheduleTriggerTask(TriggerTask task) {
        ...
        if (this.taskScheduler != null) {
            /**
             * {@link ConcurrentTaskScheduler#schedule(Runnable, Trigger)}
             * */
    ...
    public ScheduledTask scheduleCronTask(CronTask task) {
        ...
        if (this.taskScheduler != null) {
            /**
             * 属性不为null，就直接调度
             * 默认是这个 {@link ConcurrentTaskScheduler#schedule(Runnable, Trigger)}
             * */
            scheduledTask.future = this.taskScheduler.schedule(task.getRunnable(), task.getTrigger());
        } else {
            /**
             * 先存一下，在后面会给taskScheduler赋值，然后遍历存起来的东西 重新调度
             *
    ...
    public ScheduledTask scheduleFixedRateTask(FixedRateTask task) {
        ...
        if (this.taskScheduler != null) {
            if (task.getInitialDelay() > 0) {
                ...
                /**
                 * {@link ConcurrentTaskScheduler#scheduleAtFixedRate(Runnable, Date, long)}
                 * */
                scheduledTask.future =
                        this.taskScheduler.scheduleAtFixedRate(task.getRunnable(), startTime, task.getInterval());
            } else {
                /**
                 * {@link ConcurrentTaskScheduler#scheduleAtFixedRate(Runnable, long)}
                 * */
                scheduledTask.future =
    ...
    public ScheduledTask scheduleFixedDelayTask(FixedDelayTask task) {
        ...
        if (this.taskScheduler != null) {
            if (task.getInitialDelay() > 0) {
                Date startTime = new Date(this.taskScheduler.getClock().millis() + task.getInitialDelay());
                /**
                 * {@link ConcurrentTaskScheduler#scheduleWithFixedDelay(Runnable, Date, long)}
                 * */
                scheduledTask.future =
                        this.taskScheduler.scheduleWithFixedDelay(task.getRunnable(), startTime, task.getInterval());
            } else {
                /**
                 * {@link ConcurrentTaskScheduler#scheduleWithFixedDelay(Runnable, long)}
                 * */
                scheduledTask.future =
            ...
            /**
             *
             * 将 task 记录在 {@link ScheduledTaskRegistrar#fixedDelayTasks} 属性中，
             * 然后使用 task 作为key 记录其 scheduledTask
             *
             *
             * 目的是后面会对 taskScheduler 属性赋值，然后遍历属性 fixedDelayTasks 注册
             *  {@link ScheduledTaskRegistrar#afterPropertiesSet()}

...
public abstract class AbstractAspectJAdvice implements Advice, AspectJPrecedenceInformation, Serializable {
    ...
    /**
     * 值不为-1，就是有 JoinPoint 或者 ProceedingJoinPoint 类型的参数。(只支持第一个参数是这个类型)
    ...
    /**
     * 值不为-1，就是有 JoinPoint.StaticPart 类型的参数。(只支持第一个参数是这个类型)
    ...
    public final synchronized void calculateArgumentBindings() {
        ...
        /**
         * 绑定切面类上下文的参数，通过这里可以知道，我们写Advice时要想使用 必须写在参数列表的第一个
         *

...
public abstract class AbstractMessageConverterMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private static final Set<HttpMethod> SUPPORTED_METHODS = EnumSet.of(
    ...
    protected <T> Object readWithMessageConverters(HttpInputMessage inputMessage, MethodParameter parameter,
        ...
        try {
            contentType = inputMessage.getHeaders()
                    .getContentType();
        ...
        try {
            ...
            // 遍历 HttpMessageConverter 找到可以读取 这个请求体的
            for (HttpMessageConverter<?> converter : this.messageConverters) {
                Class<HttpMessageConverter<?>> converterType = (Class<HttpMessageConverter<?>>) converter.getClass();
                GenericHttpMessageConverter<?> genericConverter = (converter instanceof GenericHttpMessageConverter ? (GenericHttpMessageConverter<?>) converter : null);
                // 可读
                if (genericConverter != null ? genericConverter.canRead(
                        targetType, contextClass, contentType) : (targetClass != null && converter.canRead(
                        targetClass, contentType))) {
                    // 有请求体
                    if (message.hasBody()) {
                        // 前置操作
                        ...
                        body = (genericConverter != null ? genericConverter.read(
                                targetType, contextClass, msgToUse) : ((HttpMessageConverter<T>) converter).read(
                                targetClass, msgToUse));
                        // 后置操作
        ...
        // 解析的请求体式空，是否需要返回null
        if (body == NO_VALUE) {
            if (httpMethod == null || !SUPPORTED_METHODS.contains(
                    httpMethod) || (noContentType && !message.hasBody())) {
                return null;
            }
    ...
    protected boolean isBindExceptionRequired(WebDataBinder binder, MethodParameter parameter) {
        ...
        Class<?>[] paramTypes = parameter.getExecutable()
                .getParameterTypes();

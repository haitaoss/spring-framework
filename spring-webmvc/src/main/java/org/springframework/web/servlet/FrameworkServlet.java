...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
    ...
    protected WebApplicationContext initWebApplicationContext() {
        ...
        if (wac == null) {
            /**
             * {@link FrameworkServlet#createWebApplicationContext(ApplicationContext)}
             * 1. 创建一个IOC容器(默认是是这个 XmlWebApplicationContext，特点是会读取默认的配置文件 /WEB-INF/dispatcher-servlet.xml )
             * 2. 配置并刷新子容器
             *      configureAndRefreshWebApplicationContext(cwac);
             * */
    ...
    protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
        if (ObjectUtils.identityToString(wac)
                .equals(wac.getId())) {
            ...
            if (this.contextId != null) {
                ...
                // Generate default id...
                wac.setId(
                        ConfigurableWebApplicationContext.APPLICATION_CONTEXT_ID_PREFIX + ObjectUtils.getDisplayString(
                                getServletContext().getContextPath()) + '/' + getServletName());
            }
        }
        ...
        /**
         * 添加事件监听器。
         * SourceFilteringListener 是用来过滤掉 非wac发布的事件，
         * 因为 {@link AbstractApplicationContext#publishEvent(Object, ResolvableType)} 发布流程
         *  1. 发布事件给当前IOC容器注册的 ApplicationListener
         *  2. 存在父容器，就是用父容器也发布事件，也就是说事件的发布是有传递性的
         * */
        ...
        if (env instanceof ConfigurableWebEnvironment) {
            // 扩展IOC容器的 Environment，使其可以读到 getServletContext() 和 getServletConfig() 的属性信息
        ...
        /**
         * 拿到配置在 web容器的初始化参数，实例化成 ApplicationContextInitializer ,
         * 回调 {@link ApplicationContextInitializer#initialize(ConfigurableApplicationContext)} 方法，
         * 加工IOC容器
         * */
        ...
        try {
            ...
            if (initializerContextClass != null && !initializerContextClass.isInstance(wac)) {
                throw new ApplicationContextException(String.format(
                        ...
                        + "framework servlet: [%s]",
                        initializerClass.getName(), initializerContextClass.getName(), wac.getClass()
                                .getName()
                ));
            }
    ...
    protected final void processRequest(HttpServletRequest request,
        ...
        /**
         * 根据 request 生成 localeContext
         * {@link DispatcherServlet#buildLocaleContext(HttpServletRequest)}
         * */
        LocaleContext localeContext = buildLocaleContext(request);

        // 拿到之前的
        RequestAttributes previousAttributes = RequestContextHolder.getRequestAttributes();
        // 将 request和response 包装成 ServletRequestAttributes
        ...
        /**
         * 将 localeContext 和 requestAttributes 设置到 ThreadLocal 中

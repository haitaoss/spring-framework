...
public class RedirectView extends AbstractUrlBasedView implements SmartView {
    ...
    protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
        ...
        // 拼接好最终重定向的url。替换占位符{} + 会补上查询参数
        String targetUrl = createTargetUrl(model, request);
        /**
         * 更新最终的url。获取容器中 {@link RequestDataValueProcessor} 类型的bean，回调其方法
         * */
        targetUrl = updateTargetUrl(targetUrl, model, request, response);

        // 保存 flash 属性到session域中
     ...
     */
    @SuppressWarnings("unchecked")
    protected void appendQueryProperties(StringBuilder targetUrl, Map<String, Object> model,
                                         String encodingScheme) throws UnsupportedEncodingException {
        ...
        boolean first = (targetUrl.toString()
                .indexOf('?') < 0);
        for (Map.Entry<String, Object> entry : queryProperties(model).entrySet()) {
            Object rawValue = entry.getValue();
            Collection<?> values;
            if (rawValue != null && rawValue.getClass()
                    .isArray()) {
    ...
    protected boolean isRemoteHost(String targetUrl) {
        ...
        String targetHost = UriComponentsBuilder.fromUriString(targetUrl)
                .build()
                .getHost();

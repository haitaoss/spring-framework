...
public abstract class AbstractView extends WebApplicationObjectSupport implements View, BeanNameAware {
    ...
    public void render(@Nullable Map<String, ?> model, HttpServletRequest request,
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "View " + formatViewName() + ", model " + (model != null ? model : Collections.emptyMap()) + (this.staticAttributes.isEmpty() ? "" : ", static attributes " + this.staticAttributes));
        }

        // 就是合并 model+view+request 中的属性信息，作为视图渲染的上下文对象
        Map<String, Object> mergedModel = createMergedOutputModel(model, request, response);
        // 准备response，默认就是设置响应头
        prepareResponse(request, response);
        /**
         *
         * {@link InternalResourceView#renderMergedOutputModel(Map, HttpServletRequest, HttpServletResponse)}
    ...
    protected Map<String, Object> createMergedOutputModel(@Nullable Map<String, ?> model, HttpServletRequest request,
        ...
        @SuppressWarnings("unchecked") Map<String, Object> pathVars = (this.exposePathVariables ? (Map<String, Object>) request.getAttribute(

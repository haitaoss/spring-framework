...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver implements HandlerMethodReturnValueHandler {
    ...
    private static final Set<String> SAFE_EXTENSIONS = new HashSet<>(
            Arrays.asList("txt", "text", "yml", "properties", "csv", "json", "xml", "atom", "rss", "png", "jpe", "jpeg",
                    "jpg", "gif", "wbmp", "bmp"
            ));

    private static final Set<String> SAFE_MEDIA_BASE_TYPES = new HashSet<>(Arrays.asList("audio", "image", "video"));

    private static final List<MediaType> ALL_APPLICATION_MEDIA_TYPES = Arrays.asList(
    ...
    protected <T> void writeWithMessageConverters(@Nullable T value, MethodParameter returnType,
        ...
        if (isResourceType(value, returnType)) {
            ...
            if (value != null && inputMessage.getHeaders()
                    .getFirst(HttpHeaders.RANGE) != null && outputMessage.getServletResponse()
                    .getStatus() == 200) {
                Resource resource = (Resource) value;
                try {
                    List<HttpRange> httpRanges = inputMessage.getHeaders()
                            .getRange();
        ...
        if (selectedMediaType != null) {
            ...
            for (HttpMessageConverter<?> converter : this.messageConverters) {
                ...
                // 可写
                if (genericConverter != null ? ((GenericHttpMessageConverter) converter).canWrite(
                        targetType, valueType, selectedMediaType) : converter.canWrite(valueType, selectedMediaType)) {
                    /**
                     * 回调 beforeBodyWrite。可以在这个环节修改最终输出到response的内容
                     * {@link RequestResponseBodyAdviceChain}
                     *
                     * 是在这个地方初始化的
                     *  {@link ExceptionHandlerExceptionResolver#getDefaultReturnValueHandlers()}
                     * */
                    ...
                    if (body != null) {
                        ...
                        LogFormatUtils.traceDebug(
                                logger, traceOn -> "Writing [" + LogFormatUtils.formatValue(theBody, !traceOn) + "]");
                        addContentDispositionHeader(inputMessage, outputMessage);
                        // 将返回值内容写到 outputMessage 中
    ...
    protected List<MediaType> getProducibleMediaTypes(HttpServletRequest request, Class<?> valueClass,
        ...
        Set<MediaType> mediaTypes = (Set<MediaType>) request.getAttribute(
    ...
    private MediaType getMostSpecificMediaType(MediaType acceptType, MediaType produceType) {
        ...
        return (MediaType.SPECIFICITY_COMPARATOR.compare(
                acceptType, produceTypeToUse) <= 0 ? acceptType : produceTypeToUse);
    }
    ...
    private void addContentDispositionHeader(ServletServerHttpRequest request, ServletServerHttpResponse response) {
        ...
        try {
            int status = response.getServletResponse()
                    .getStatus();
    ...
    private MediaType resolveMediaType(ServletRequest request, String extension) {
        ...
        String rawMimeType = request.getServletContext()
        ...
        if (result == null || MediaType.APPLICATION_OCTET_STREAM.equals(result)) {
            result = MediaTypeFactory.getMediaType("file." + extension)
                    .orElse(null);
        }
        return result;
    }
    ...
    private boolean safeMediaType(MediaType mediaType) {
        return (SAFE_MEDIA_BASE_TYPES.contains(mediaType.getType()) || mediaType.getSubtype()
                .endsWith("+xml"));
    }

...
public abstract class AbstractDispatcherServletInitializer extends AbstractContextLoaderInitializer {
    ...
    public void onStartup(ServletContext servletContext) throws ServletException {
         ...
         * 很简单。
         *
         * 如果存在 RootConfigClasses 就创建出一个 IOC容器，并构造出 Listener 装饰，然后将 Listener 注册到 servletContext 中
        ...
        /**
         * 1. 创建一个IOC容器(子容器)
         * 2. 创建 DispatcherServlet，将子容器暴露给 DispatcherServlet，并将 DispatcherServlet 注册到 web容器中
         * */
    ...
    protected void registerDispatcherServlet(ServletContext servletContext) {
        ...
        /**
         * 1. 创建一个IOC容器 AnnotationConfigWebApplicationContext
         * 2. 如果存在配置类，就将配置类的信息设置的容器中
         * {@link AbstractAnnotationConfigDispatcherServletInitializer#createServletApplicationContext()}
         * */
         ...
         * {@link AbstractDispatcherServletInitializer#createDispatcherServlet(WebApplicationContext)}
         * */
        FrameworkServlet dispatcherServlet = createDispatcherServlet(servletAppContext);
        Assert.notNull(dispatcherServlet, "createDispatcherServlet(WebApplicationContext) must not return null");
        // 设置容器初始化器。是用来在refresh之前 加工IOC容器的
        ...
        if (!ObjectUtils.isEmpty(filters)) {
            for (Filter filter : filters) {
                // 注册 Filter，Filter 不是根据路径拦截的，而是根据servletName，注册的Filter子拦截当前 dispatcherServlet

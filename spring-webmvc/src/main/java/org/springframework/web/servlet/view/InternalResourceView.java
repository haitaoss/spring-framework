...
public class InternalResourceView extends AbstractUrlBasedView {
    ...
    /**
     * 是否需要判断 当前转发的路径是不是死循环了
     * {@link InternalResourceView#prepareForRendering(HttpServletRequest, HttpServletResponse)}
     */
    ...
    protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
        ...
        if (rd == null) {
            throw new ServletException(
                    "Could not get RequestDispatcher for [" + getUrl() + "]: Check that the corresponding file exists within your web application archive!");
        }
    ...
    protected String prepareForRendering(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (this.preventDispatchLoop) {
            ...
            // 防止死循环。路径和当前请求路径一直说明循环了
            if (path.startsWith("/") ? uri.equals(path) : uri.equals(StringUtils.applyRelativePath(uri, path))) {
                throw new ServletException(
                        "Circular view path [" + path + "]: would dispatch back " + "to the current handler URL [" + uri + "] again. Check your ViewResolver setup! " + "(Hint: This may be the result of an unspecified view, due to default view name generation.)");
            }
        }
        return path;
    }

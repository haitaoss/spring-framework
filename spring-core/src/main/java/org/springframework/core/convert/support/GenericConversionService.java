...
public class GenericConversionService implements ConfigurableConversionService {
    ...
    private static class Converters {
        ...
        public GenericConverter find(TypeDescriptor sourceType, TypeDescriptor targetType) {
            ...
            for (Class<?> sourceCandidate : sourceCandidates) {
                for (Class<?> targetCandidate : targetCandidates) {
                    ...
                    /**
                     * 找到匹配的就返回，很简单从 {@link Converters#converters} 这个map中找到匹配的,
                     * 怎么匹配的，看重写的equals方法 {@link ConvertiblePair#equals(Object)} 就知道了，简单的很
                     * */
        ...
        private List<Class<?>> getClassHierarchy(Class<?> type) {
            ...
            /**
             *
             * visited 是记录处理过的，只有没处理过才会添加到 hierarchy
             *
             * 将当前类存到 hierarchy(如果是基本类型会存其包装类)
             * */
            ...
            int i = 0;
            // while循环拿到 所有的父类，所有的接口
            while (i < hierarchy.size()) {
                Class<?> candidate = hierarchy.get(i);
                /**
                 * 是数组就返回其元素的类型，否则就返回当前类型(如果是基本数据类型就返回其包装类)
                 * */

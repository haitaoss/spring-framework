...
public abstract class AnnotationUtils {
    ...
    /**
     * annotationTypes 是 java包下的，就是true。clazz是java包下的 或者 是Ordered类型就是false。
    ...
    public static boolean isCandidateClass(Class<?> clazz, Collection<Class<? extends Annotation>> annotationTypes) {
        for (Class<? extends Annotation> annotationType : annotationTypes) {
            /**
             * annotationType 是 java包下的，就是true。
             * clazz是java包下的 或者 是Ordered类型就是false
             * */
    ...
    /**
     * 是候选类。
     * 注解是在 java包下 return true.
     * 类是java包下 或者 类是 Ordered return false.
     * 其余情况就返回true
     * (type.getName().startsWith("java.") || type == Ordered.class);

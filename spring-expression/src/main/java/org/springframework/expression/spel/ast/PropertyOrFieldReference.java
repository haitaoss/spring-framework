    ...
    private TypedValue readProperty(TypedValue contextObject, EvaluationContext evalContext, String name)
        ...
        try {
            for (PropertyAccessor accessor : accessorsToTry) {
                /**
                 * 判断可以读取
                 * {@link org.springframework.context.expression.BeanExpressionContextAccessor#canRead(EvaluationContext, Object, String)}
                 *  会判断容器中是否有这个 value 这个bean
                 * */
                if (accessor.canRead(evalContext, contextObject.getValue(), name)) {
                    ...
                    /**
                     * 返回读取的结果
                     * {@link org.springframework.context.expression.BeanExpressionContextAccessor#read(EvaluationContext, Object, String)}

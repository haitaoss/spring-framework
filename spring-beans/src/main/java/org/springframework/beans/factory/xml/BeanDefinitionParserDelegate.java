...
public class BeanDefinitionParserDelegate {
                ...
                try {
                    if (containingBean != null) {
                        ...
                        if (beanClassName != null && beanName.startsWith(beanClassName)
                            && beanName.length() > beanClassName.length() && !this.readerContext.getRegistry()
                    ...
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Neither XML 'id' nor 'name' specified - " + "using generated bean name [" + beanName
                                + "]");
                    }
                } catch (Exception ex) {
    ...
    public Properties parsePropsElement(Element propsEle) {
        ...
        for (Element propEle : propEles) {
            ...
            String value = DomUtils.getTextValue(propEle)
                    .trim();
    ...
    public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
        ...
        // 根据命名空间映射到对应的那个 handler处理类 META-INF/spring.handlers
        ...
        // 根据 ele 找到对应的 Parse，在使用Parser解析标签
        return handler.parse(ele, new ParserContext(this.readerContext, this, containingBd)); // org.springframework.beans.factory.xml.NamespaceHandlerSupport.parse
    }

...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
    ...
    /**
     * 一级缓存这个就是我们大名鼎鼎的单例缓存池用于保存我们所有的单实例bean
     * Cache of singleton objects: bean name to bean instance.
     */
    private final Map<String, Object> singletonObjects = new ConcurrentHashMap<>(256);

    /**
     * 三级缓存该map用户缓存key为 beanName value为 ObjectFactory(包装早期对象的创建方式 Lambda 表达式，主要目的是提前aop)
     * Cache of singleton factories: bean name to ObjectFactory.
     */
    private final Map<String, ObjectFactory<?>> singletonFactories = new HashMap<>(16);

    /**
     * 二级缓存，用户缓存我们的key为 beanName value是我们的早期对象（对象属性还没有来得及进行赋值，提前aop的对象）
     ...
     *
     * 原因：获取三级缓存--getEarlyBeanReference() 经过一系列的后置处理来给我们早期对象进行特殊化处理 (提前aop)
     *      从三级缓存中获取包装对象的时候，他会经过一次后置处理器的处理,会对我们早期对象的bean进行
     *      特殊化处理，但是 spring的原生后置处理器没有经过处理，而是留给了我们程序员进行扩展
     *      singletonObject = singletonFactory.getObject();
     *      把三级缓存移值到二级缓存中 this.earlySingletonObjects.put(beanName, singletonObject);
     *      删除三级缓存 this.singletonFactories.remove(beanName);
    ...
    protected Object getSingleton(String beanName, boolean allowEarlyReference) {
        /**
         * 第一步：我们尝试去一级缓存（单例缓存池中去获取对象，一股情况从该m中获取的对象是直接可以使用的）
         * IOC容器初始化加我单实例bean的时候第一次进来的时候该map中一般返回空
         * */
        // Quick check for existing instance without full singleton lock
        Object singletonObject = this.singletonObjects.get(beanName);
        /**
         * 若在第一级级存中没有获取到对象，并且 singletonsCurrentlyInCreation 这个list包含该 beanName
         * IOC容器初始化加我单实例bean的时候第一次进来的时候该list中一般返回空，但是循环依赖的时候可以满足该条件
         * */
        if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
            /**
             * 尝试去二级缓存中获取对象（二级缓存中的对象是一个早期对象，是通过三级缓存执行提前aop后的对象）
            ...
            if (singletonObject == null && allowEarlyReference) {
                synchronized (this.singletonObjects) {
                    ...
                    if (singletonObject == null) {
                        ...
                        if (singletonObject == null) {
                            /**
                             * 直接从三级緩存中获取 ObjectFactory对象这个就是用来解决循环依赖的关键所在
                             * 在ioc后期的过程中，当bean调用了构造方法的时候，把早期对象包裹成一个 ObjectFactory 暴露到三级缓存中
                             *
                             * 设置三级缓存的代码：
                             *     执行下面这个方法的逻辑：是单例bean && this.allowCircularReferences && isSingletonCurrentlyInCreation(beanName)
                            ...
                            if (singletonFactory != null) {
                                /**
                                 * 在这里通过暴露的 ObjectFactory包装对象中，通过调用他的 getObject()来获取我们的早期对象(提前aop的bean对象)
        ...
        synchronized (this.singletonObjects) {
            ...
            if (singletonObject == null) {
                ...
                try {
                    ...
                    // 回调我们 singletonObjects的get方法，进行正在的创建bean的逻辑
                    ...
                    /**
                     * <4>后置处理
                     * 主要做的事情就是把 singletonsCurrentlyInCreation标记正在创建的bean从集合中移除
                     */
    ...
    public void registerDependentBean(String beanName, String dependentBeanName) {
        // 获取依赖bean最原始的名字，因为可能去了别名，别名又去了别名的情况
        String canonicalName = canonicalName(beanName);

        /**
         * @see cn.haitaoss.javaconfig.dependson.A
         * A 依赖 C
         * B 依赖 C
         * beanName: C , dependentBeanName: A和B
         * dependentBeanMap:
         *  key: C
         *  value: {A,B}
         *

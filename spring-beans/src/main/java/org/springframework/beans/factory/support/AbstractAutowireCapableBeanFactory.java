...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory implements AutowireCapableBeanFactory {
    ...
    public Object applyBeanPostProcessorsAfterInitialization(Object existingBean,
        ...
        for (BeanPostProcessor processor : getBeanPostProcessors()) {
            /**
             * 在这里是后置处理器的【第九次调用】
             * TODOHAITAO 后置处理器的【第九次】BeanPostProcessor#postProcessAfterInitialization
    ...
    /**
     * TODOHAITAO 创建bean的流程，bean的生命周期
    ...
    protected Object doCreateBean(String beanName, RootBeanDefinition mbd,
        ...
        /**
         * TODOHAITAO 该对象进行判断是否能够暴露早期对象的条件
    ...
    protected Class<?> determineTargetType(String beanName, RootBeanDefinition mbd, Class<?>... typesToMatch) {
        ...
        if (targetType == null) {
            targetType = (mbd.getFactoryMethodName() != null ? getTypeForFactoryMethod(
                    beanName, mbd, typesToMatch) : resolveBeanClass(mbd, beanName, typesToMatch));
    ...
    protected Class<?> getTypeForFactoryMethod(String beanName, RootBeanDefinition mbd, Class<?>... typesToMatch) {
        ...
        cachedReturnType = (uniqueCandidate != null ? ResolvableType.forMethodReturnType(
                uniqueCandidate) : ResolvableType.forClass(commonType));
    ...
    protected ResolvableType getTypeForFactoryBean(String beanName, RootBeanDefinition mbd, boolean allowInit) {
        ...
        ResolvableType beanType = (mbd.hasBeanClass() ? ResolvableType.forClass(
                mbd.getBeanClass()) : ResolvableType.NONE);
        ...
        if (allowInit) {
            FactoryBean<?> factoryBean = (mbd.isSingleton() ? getSingletonFactoryBeanForTypeCheck(
    ...
    private ResolvableType getFactoryBeanGeneric(@Nullable ResolvableType type) {
        ...
        return type.as(FactoryBean.class)
                .getGeneric();
    }
    ...
    protected Object getEarlyBeanReference(String beanName, RootBeanDefinition mbd, Object bean) {
        Object exposedObject = bean;
        // 判读我们容器中是否有 InstantiationAwareBeanPostProcessors 类型的后置处理器
    ...
    protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
         ...
         * @see cn.haitaoss.javaconfig.service.MyBeanFactoryPostProcessorr#postProcessBeanDefinitionRegistry(BeanDefinitionRegistry)
         * */
    ...
    /**
     * 实例化Bean，会使用 InstantiationStrategy 来实例化bean
     *
    ...
    protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
        ...
        if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
            for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
                /**
                 * TODOHAITAO 后置处理器的【第五次】InstantiationAwareBeanPostProcessor#postProcessAfterInstantiation
        ...
        if (hasInstAwareBpps) {
            ...
            for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
                /**
                 * TODOHAITAO 后置处理器的【第六次】InstantiationAwareBeanPostProcessor#postProcessProperties
    ...
    protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
        ...
        for (PropertyDescriptor pd : pds) {
            /**
             * isExcludedFromDependencyCheck 会使用属性 {@link AbstractAutowireCapableBeanFactory#ignoredDependencyInterfaces} 判断
             * PropertyDescriptor 是想给忽略的依赖设置值，就不要记录，排除掉。
             *
             * 注：
             *  - PropertyDescriptor 描述的是 get、set方法。比如XxxAware接口的set方法，不应该通过PropertyDescriptor设置值，而是通过ApplicationContextAwareProcessor这个后置处理器回调设置的
             *  - 在这里设置的忽略依赖 {@link org.springframework.context.support.AbstractApplicationContext#prepareBeanFactory(ConfigurableListableBeanFactory)}
             * */
    ...
    /**
     * PropertyDescriptor 是 忽略依赖的，就返回true。含义就是不能通过 PropertyDescriptor 给忽略依赖设置值，
     * 因为这个值是通过ApplicationContextAwareProcessor设置的
        ...
        for (PropertyValue pv : original) {
            if (pv.isConverted()) {
                ...
                if (originalValue == AutowiredPropertyMarker.INSTANCE) {
                    Method writeMethod = bw.getPropertyDescriptor(propertyName)
                            .getWriteMethod();
                    if (writeMethod == null) {
                ...
                boolean convertible = bw.isWritableProperty(
                        propertyName) && !PropertyAccessorUtils.isNestedOrIndexedProperty(propertyName);
                if (convertible) {
                ...
                if (resolvedValue == originalValue) {
                    ...
                    deepCopy.add(pv);
                } else if (convertible && originalValue instanceof TypedStringValue && !((TypedStringValue) originalValue).isDynamic() && !(convertedValue instanceof Collection || ObjectUtils.isArray(
                        convertedValue))) {
                    pv.setConvertedValue(convertedValue);
                    deepCopy.add(pv);
                } else {
    ...
    protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
        if (System.getSecurityManager() != null) {
            ...
            // TODOHAITAO 若我们的bean实现了 XXXAware接口进行方法的回调
            invokeAwareMethods(beanName, bean);
        }
    ...
    protected void invokeCustomInitMethod(String beanName, Object bean, RootBeanDefinition mbd) throws Throwable {
        ...
        Method initMethod = (mbd.isNonPublicAccessAllowed() ? BeanUtils.findMethod(
                bean.getClass(), initMethodName) : ClassUtils.getMethodIfAvailable(bean.getClass(), initMethodName));
    ...
    private static class FactoryBeanMethodTypeFinder implements MethodCallback {
        ...
        public void doWith(Method method) throws IllegalArgumentException, IllegalAccessException {
            if (isFactoryBeanMethod(method)) {
                ...
                ResolvableType candidate = returnType.as(FactoryBean.class)
                        .getGeneric();
        ...
        private boolean isFactoryBeanMethod(Method method) {
            return (method.getName()
                    .equals(this.factoryMethodName) && FactoryBean.class.isAssignableFrom(method.getReturnType()));
        }
